...
The update command extracts the core logic into `runUpdateCycle()` to enable retry handling:
...
When running from the hook (`--from-hook` flag), the update command implements a retry mechanism to handle commits that occur during wiki generation:
...
The `hasUnprocessedCommits()` function checks if there are code changes after the last processed commit:
...
1. Commit A triggers the hook
2. Hook acquires lock and starts wiki generation
3. Commit B occurs during generation (its hook exits due to lock)
4. Generation for Commit A completes and releases lock
5. Retry loop detects unprocessed commits (Commit B)
6. Update cycle runs again to process Commit B
